"""
Modified base_task file of CausalWorld
The functions that were modified from original was:
    __init__() function:
        To allow the use for causal_rep
        Added extra parameters and properties for the class.
    filter_structured_observation:
        Added code to append causal_rep to structured ...
        observation.
"""
import numpy as np
import math
...
class BaseTask(object):
    ...
    def __init__(self,
        ...
        :param mode: (int) specify if there will be causal_rep attached to obs
        :param causal_rep: (nd.array) Will store the causal rep that will be concat to obs
        """
        self._robot = None
        self._stage = None
        ...
        # If mode = 1 then causal_rep is used, else if mode = 0 then no causal_rep used.
        self._own_CW_mode = mode
        self._own_CW_causal_rep = causal_rep
        # low_cRep and high_cRep is used by the causalworld.py to ...
        # initialize causal_rep when the environment resets.
        self._low_val_cRep = low_cRep
        self._high_val_cRep = high_cRep
        self._shape_cRep = shape_cRep
        return
    ...
    def filter_structured_observations(self):
        ...
        # Own Part: where causal rep can be concatenated
        if self._own_CW_mode == 1:
            observations_filtered = \
                np.append(observations_filtered,
                self._own_CW_causal_rep)
