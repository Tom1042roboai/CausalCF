"""
Modified the causalworld class for CausalCF.
The functions that were modified from original was:
    _reset_observation_space_():
        Added additional code to check if a causal_rep is used.
        if used then initialize a causal_rep and concat to obs.
Function that is added to the original class:
    update_causal_rep():
        This function is used by CausalCF (iter) solution to update ...
        the causal_rep during training.
"""
import numpy as np
import gym
import pybullet
        ...
        if self._observation_mode == "pixel" and self.observation_space is None:
            ...
            self.observation_space = \
                combine_spaces(self._robot.get_observation_spaces(),
                               self._stage.get_observation_spaces())
            if self._task._own_CW_mode == 1:
                total_size_cRep = self._task._shape_cRep[0] * self._task._shape_cRep[1]
                addedBox = spaces.Box(low=self._task._low_val_cRep,
                                    high=self._task._high_val_cRep,
                                    shape=(total_size_cRep,),
                                    dtype=np.float64)
                self.observation_space = \
                    combine_spaces(self.observation_space,
                                    addedBox)
        return
...
### OWN FUNCTION ####
    def update_causal_rep(self, new_causal_rep):
        self._task._own_CW_causal_rep = new_causal_rep