...
class BaseTask(object):
    ...
    def __init__(self,
        ...
        self._task_params["mode"] = mode
        self._task_params["reference"] = reference

        self._task_params["tool_block_mass"] = mass
        self._task_params["tool_block_shape"] = shape
        self._task_params["tool_block_size"] = size
    ...
    def filter_structured_observations(self):
        ...
        # print("self._task_params", self._task_params["tool_block_mass"])
        if self._task_params["mode"]==1:
            if self._task_params["tool_block_mass"] >= self._task_params["reference"]["mass"][len(self._task_params["reference"]["mass"]) // 2]:
                massRep = 1
            observations_filtered = \
                    np.append(observations_filtered,
                              np.array(massRep))
        if self._task_params["mode"] ==2:
            if self._task_params["tool_block_size"] >= self._task_params["reference"]["size"][len(self._task_params["reference"]["size"]) // 2]:
                sizeRep = 1
            observations_filtered = \
                    np.append(observations_filtered,
                              np.array(sizeRep))
        if self._task_params["mode"] ==3:
            if self._task_params["tool_block_mass"] >= self._task_params["reference"]["mass"][len(self._task_params["reference"]["mass"]) // 2]:
                massRep = 1
            if self._task_params["tool_block_size"] >= self._task_params["reference"]["size"][len(self._task_params["reference"]["size"]) // 2]:
                sizeRep = 1
